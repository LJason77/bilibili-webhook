...
RUN --mount=type=cache,target=/var/cache/apk,id=alpine_apk apk add -qq --update-cache --repository=https://dl-cdn.alpinelinux.org/alpine/edge/community musl-dev libc6-compat openssl-dev sqlite-dev tzdata
...
RUN --mount=type=cache,target=/var/cache/apk,id=alpine_apk \
